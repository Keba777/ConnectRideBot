...
async def handle_rating(update: Update, context: CallbackContext) -> int:
    if update.message.text.strip().lower() == "⭐️ rate us":
...
async def handle_rating_input(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        if 1 <= rating <= 5:
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text="Awesome! Now, share your feedback about the ride.")
            return FEEDBACK
        else:
